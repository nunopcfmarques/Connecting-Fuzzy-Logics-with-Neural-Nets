...
def ast_to_ReLU(root: Tree.Node, max_depth: int, connectives_to_ReLU: dict) -> ReLUNetwork:
    ...
    while expand_queue:
        ...
        if ReLU_v.weights:
            if connective == "δ":
                ReLU_v.vertically_append_ReLUs(connectives_to_ReLU[connective](node.data[1:]))
            else:
                ReLU_v.vertically_append_ReLUs(connectives_to_ReLU[connective])
        else:
            if connective == "δ":
