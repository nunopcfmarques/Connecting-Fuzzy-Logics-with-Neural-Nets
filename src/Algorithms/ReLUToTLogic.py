...
def sigma_construct(w, b):
    if np.all(w == 0):
       return str(int(sigma_activation(b)))
        ...
        w[w != 0] *= -1
        b *= -1
        return f"(¬{sigma_construct(w, b + 1)})"
    
    else:
        idx = np.where(w > 0)[0][0]
        wl, wr = deepcopy(w), deepcopy(w)
...
def CReLU_to_formula(CReLU: ReLUNetwork) -> str:
    ...
    return MV_terms[CReLU.num_layers - 1][1], MV_terms #last layer should only have one neuron
